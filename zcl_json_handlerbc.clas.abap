class ZCL_JSON_HANDLERBC definition
  public
  create public .

public section.

*"* public components of class ZCL_JSON_HANDLERBC
...
*"* do not include other source files here!!!
  ...
  methods CALL_FUNCTION
    importing
      ...
      !QS_NVP type TIHTTPNVP optional
      !FORMAT type STRING default 'JSON'
      !JSONP_CALLBACK type STRING optional
...
CLASS ZCL_JSON_HANDLERBC IMPLEMENTATION.
...
*****
*
  if remote_system is not initial.

    data ret_ctype type string.

    CALL FUNCTION 'ZRFC_GENERIC_CALL' DESTINATION REMOTE_SYSTEM
      EXPORTING
        PARAMTAB_STR       = input_data
        FUNCNAME           = funcname
        FORMAT             = format
...
*/************************************************/*
    ...
    if sy-subrc eq 0.
      ...
      <fm_int_handler>-path_info = me->server->request->get_header_field( name = '~path_info' ).
      <fm_int_handler>-qs_tab = qs_nvp.
      <fm_int_handler>-i_json_data = input_data.
      append '_ICF_DATA' to <fm_int_handler>-delete_params.
      <fm_int_handler>-server = me->server. " Beware!
    endif.
...
*/***********************************/*
    if <fm_int_handler> is assigned.
      if <fm_int_handler>-http_code is not initial.
        me->server->response->set_status( code = <fm_int_handler>-http_code  reason = <fm_int_handler>-http_status ).
      ...
      if <fm_int_handler>-error_message is not initial.
        ...
        me->http_error( http_code = str_item status_text = <fm_int_handler>-http_status message = <fm_int_handler>-error_message ).
      endif.
      ...
      when 'YAML'.
        ...
        call method me->serialize_yaml
          exporting
            ...
            lowercase   = lowercase
          importing
            yaml_string = output_data.

        me->server->response->set_header_field( name = 'Content-Type' value = 'text/plain' ).
...
*      CALL METHOD me->serialize_id
          exporting
            ...
            format    = format
          importing
            o_string  = output_data.

        me->server->response->set_header_field( name = 'Content-Type' value = 'application/xml' ).
...
*           format    = format
        ...
        me->server->response->set_header_field( name = 'Content-Type' value = 'application/json' ).
        if jsonp_callback is not initial.
          me->server->response->set_header_field( name = 'Content-Type' value = 'application/javascript' ).
        endif.
...
* inside the FM (search for _ICF_DATA in the call_function method).
...
* Get function name from PATH_INFO
  ...
  if funcname is initial and action is initial.
    http_error( exporting http_code = '404' status_text = 'Not Found' message = 'Empty request.' ).
  endif.
...
* See http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html. Thanks Uwe!!
  if accept is not initial and format is initial.
    ...
    elseif accept cs '*/*'.
      ...
      http_error( exporting http_code = '406' status_text = 'Not Acceptable' message = 'The server cannot send a response which is acceptable according to the combined Accept field value' ).
    endif.
  endif.
...
* Figure out how to do the batch...
  if funcname eq '$BATCH'.
     call method run_batch( exporting input = i_cdata ).
  else.
